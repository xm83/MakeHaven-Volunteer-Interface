...
def checkin_none():
    user = db.execute(
        "SELECT * FROM users WHERE id = :u", u=session["user_id"])
    return render_template("checkin_none.html", admin=user[0]["admin"])
...
def checkin():
    ...
    if request.method == "GET":
        ...
        if not owned_tasks:
            ...
            return render_template("checkin.html", owned_tasks=owned_tasks, admin=user[0]["admin"])
        ...
        # Add new active task if recurrence_cycle exists
        info = db.execute("SELECT * FROM tasks WHERE task = :t", t=request.form.get("task"))
        if info[0]["recurrence_cycle"] != "N/A":
            old_date = info[0]["start_date"]
            Date = datetime.strptime(old_date, "%Y-%m-%d")
            cycle = int(info[0]["recurrence_cycle"])
            new_date = Date + timedelta(days=cycle)

            db.execute("INSERT INTO tasks (task, minutes, task_description, task_score, recurrence_cycle, start_date) VALUES (:t, :m, :d, :s, :r, :date)", t=request.form.get("task"), m=info[0]["minutes"], d=info[0]["task_description"], s=info[0]["task_score"], r=info[0]["recurrence_cycle"], date=new_date)
